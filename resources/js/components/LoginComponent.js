...
const LoginComponent = () => {
  ...
  const handleLogin = async (event) => {
    ...
    try {
      ...
      // Asegúrate de que la respuesta contiene JSON adecuado.
      if (response.ok) {
        Swal.fire({
          icon: 'success',
        ...
        throw new Error(result.message || 'Correo Electrónico o Contraseña incorrectos.');
      }
    } catch (error) {
      Swal.fire({
        icon: 'error',
        ...
        showConfirmButton: true,
        confirmButtonText: 'Aceptar',
        confirmButtonClass: 'bg-blue-00 text-white px-5 py-2 rounded-lg hover:bg-blue-700'
      });
    }
  };
  ...
  return (
    <div className="login flex justify-end items-center xl:pt-5 xl:pb-10 xl:pr-32 bg h-fit">
      <div className="flex flex-col items-center justify-center w-1/3 h-full xl:space-y-12">
        ...
        <form onSubmit={handleLogin} className='form-container flex flex-col items-center bg-white xl:px-12 xl:py-10 w-full xl:space-y-6 xl:rounded-xl shadow-sm shadow-black h-[calc(100vh-42vh)] overflow-auto'>
